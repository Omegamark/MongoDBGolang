...
func AddGameToGamerGamelist(databaseName string, collectionName string, updateInfo map[string]interface{}) {
	collection := MongoClient().Database(databaseName).Collection(collectionName)
	filter := bson.M{
		"name": updateInfo["name"],
	}

	var gameStruct models.Game

	gameInfo := []byte(updateInfo["game"])

	json.Unmarshal(gameInfo, &gameStruct)

	game := models.Game{
		Name:     gameInfo["name"].(string),
		Genre:    gameInfo["genre"].(string),
		Platform: gameInfo["platform"].(string),
	}
