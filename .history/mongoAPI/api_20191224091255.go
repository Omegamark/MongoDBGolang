...
func (rm *RealMongoClient) MongoClient() *mongo.Client {
	// TODO : Make the URI here configurable.
	var dataAccess DataAccess = RealMongoClient{}
	client, err := dataAccess.Connect(context.TODO(), options.Client().ApplyURI("mongodb://localhost:27017"))
	if err != nil {
		log.Fatal(err)
	}
...
func FindOneInCollection(databaseName string, collectionName string, gamerName interface{}, projections []interface{}) (models.Gamer, error) {
	var dataAcess DataAccess
	collection := dataAccess.MongoClient().Database(databaseName).Collection(collectionName)
