...
func AddToCollection(collection *mongo.Collection, gamer ...models.Gamer) error {
	// TODO: Make the clientDB into an interface so these methods could be used with any DB.
	if len(gamer) > 1 {
		coercedSlice := []interface{}(gamer)
		_, err := collection.InsertMany(context.TODO(), []interface{}{gamer})
		if err != nil {
			return err
		}
		return nil
	}
	_, err := collection.InsertOne(context.TODO(), []interface{}{gamer[1]})
	if err != nil {
		return err
	}
	return nil
