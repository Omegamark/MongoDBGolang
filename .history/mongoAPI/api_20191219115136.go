...
// UpdateOneGamer allows the gamers name and age to be changed.
func UpdateOneGamer(databaseName string, collectionName string, filter string, update interface{}) {
	collection := MongoClient().Database(databaseName).Collection(collectionName)
	filter := bson.M{
		"name": "Sean",
	}
