...
func main() {
	...
	// updateResult, err := gamerCollection.UpdateMany(context.TODO(), filter, update)
	// if err != nil {
	// 	fmt.Println("Failed to update: ", err)
	// }
	// opts := options.FindOne().SetSort(bson.D{{"name", 1}})

	var test bsontype.String

	// func Convert(value bson.D) (bsontype.Type, []byte, error) {
	// 	return bsontype.String, bsoncore.AppendString(nil, `fixed value`), nil
	// }

	var result models.Gamer
	err := gamerCollection.FindOne(context.TODO(), bson.DocElem{{"name", "Sean"}}).Decode(&result)
	if err != nil {
		fmt.Println(err)
	}
